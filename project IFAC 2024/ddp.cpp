	...
	while (true) {
		...
		for (int k = numsteps; k >= 0; k--) {
			...
			while (true) {
				...
				// if (ayUB(yPoint, vyPoint, ayPoint, 0.0, 0.0, 0.0) >= 0.0) {
				// 	if (CtTemp[0] == 0.0 && CtTemp[1] == 0.0) {
				// 		CtTemp[0] = 0.0; CtTemp[1] = -1.0;
				// 		dTemp[0]= ayUB(yPoint, vyPoint, ayPoint, 0.0, 0.0, 0.0);
				// 		RTemp[0] = 0.0; RTemp[1] = Klat; RTemp[2] = 0.0; RTemp[3] = 2*Sqrt(Klat) + (Klat*T)/2.;
				// 	}
				// 	else {
				// 		CtTemp.push_back(0.0); CtTemp.push_back(-1.0);
				// 		dTemp.push_back(ayUB(yPoint, vyPoint, ayPoint, 0.0, 0.0, 0.0));
				// 		RTemp.push_back(0.0); RTemp.push_back(Klat); RTemp.push_back(0.0); RTemp.push_back(2*Sqrt(Klat) + (Klat*T)/2.);
				// 	}
				// 	// cout << "(" << k << ")" << " constraint violated: " << ayPoint << " | constraint value: " << ayUpSmooth(ayPoint, vyPoint, yPoint, 0.0, 0.0)  << endl;
				// }

				// if (ayLB(yPoint, vyPoint, ayPoint, 0.0, 0.0, 0.0) >= 0.0) {
				// 	if (CtTemp[0] == 0.0 && CtTemp[1] == 0.0) {
				// 		CtTemp[0] = 0.0; CtTemp[1] = 1.0;
				// 		dTemp[0]= ayLB(yPoint, vyPoint, ayPoint, 0.0, 0.0, 0.0);
				// 		RTemp[0] = 0.0; RTemp[1] = -Klat; RTemp[2] = 0.0; RTemp[3] = -2*Sqrt(Klat) + (Klat*T)/2.;
				// 	}
				// 	else {
				// 		CtTemp.push_back(0.0); CtTemp.push_back(1.0);
				// 		dTemp.push_back(ayLB(yPoint, vyPoint, ayPoint, 0.0, 0.0, 0.0));
				// 		RTemp.push_back(0.0); RTemp.push_back(-Klat); RTemp.push_back(0.0); RTemp.push_back(-2*Sqrt(Klat) + (Klat*T)/2.);
				// 	}
				// }
				
				// if no constraint is active initialize related vectors and matrices
				// if(dTemp.empty()) {
				// 	CtTemp.push_back(0.0); CtTemp.push_back(0.0);
				// 	dTemp.push_back(0.0);
				// 	RTemp.push_back(0.0); RTemp.push_back(0.0); RTemp.push_back(0.0); RTemp.push_back(0.0);
			...
			if (lambdaStar(0) > 0.0) {
				fprintf(stderr, " -- (%d) lambdaStar: %.4f | pStar: %.4f \n", k, lambdaStar(0), pStar(0));
				// CtTemp.clear(); dTemp.clear(); RTemp.clear();
