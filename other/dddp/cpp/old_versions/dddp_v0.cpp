...
#define ACCESS(M, state) M[state.k][state.ix][state.iy][state.ivx][state.ivy]
...
class Data {
	public:
		...
		double x0, y0, vx0, vy0, ax0, ay0; // is this needed?
		double vdx, vdy;
		...
		// output
		vector<double> x, y, vx, vy, ax, ay; // is this needed?

		// functions
		void input();
		void exportSimData(class Vehicle veh, class Infrastructure inf);
...
class Vehicle {
	...
	private:
		...
		double VYMIN = -3.0, VYMAX = 3.0;

	public:
		vector<double> init_vx, init_vy, init_x, init_y;
		vector<double> x, y, vx, vy, ax, ay;
	
		// Parametrized Constructor
		Vehicle(double x, double y, double vx, double vy) { 
			x0 = x; y0 = y; vx0 = vx; vy0 = vy;
		}
		
		// getter functions
		double getX0() { return x0; }				double getY0() { return y0; }
		double getVX0() { return vx0; }				double getVY0() { return vy0; }
		...
		double getVYmin() { return VYMIN; } 		double getVYmax() { return VYMAX; }
		//setter functions
	...
	int collision(Data data, double here_x, double here_y, double here_vx, double here_vy, int here_k);
};
...
class Controller {
	...
	public:
		...
		string method;
		// domain parameters
		vector<int> NX, NVX, NY, NVY;
		int maxNX, maxNVX, maxNY, maxNVY;
		vector<double> xmin, xmax, vxmin, vxmax, ymin, ymax, vymin, vymax;
		vector<vector<double>> X, VX, Y, VY;
		vector<double> UX, UY;
		double dx, dvx, dux, dy, dvy, duy;
		double uxmin, uxmax, uymin, uymax;
		int NUX, NUY;
		// corridor parameters
		double dux0 = 1.0, duy0 = 1.0, cvx0 = 4.0, cvy0 = 1.0, cxm = 5.0, cym = 10.0;
		double cx, cy, cvx, cvy;
		// optimal trajectories
		vector<double> opt_x, opt_vx, opt_y, opt_vy;
		// 
	...
	void printSolution(Vehicle v, Data data, Infrastructure inf);
	...
	node_t *getnext(node_t here, double ux, double uy){
		...
		next.y = next._y = here.y + here.vy*step + 0.5*uy*pow(step, 2);
		next.vx = next._vx = here.vx + ux*step;
		next.vy = next._vy = here.vy + uy*step;
		
		if ((err = discretizexv(&next, next.k))) {
			fprintf(stderr, " -- INVALID STATE: form (%.3f | %.3f | %.3f | %.3f) to (%.3f | %.3f | %.3f | %.3f) at time step %d to %d \n", 
				here.x, here.y, here.vx, here.vy, next.x, next.y, next.vx, next.vy, here.k, next.k);
			return NULL;
		}
		return &next;
	};
...
void Vehicle::initial_path(Data data) {
	...
	init_vx.push_back(vx0); init_vy.push_back(vy0);	
	double T = data.step;
	...
	for (int k = 0; k < data.numsteps; k++) {
		...
		init_y.push_back(init_y[k] + init_vy[k]*T);
		init_vx.push_back(init_vx[k] + uxConst*T);
		init_vy.push_back(init_vy[k]);
	}
...
int Vehicle::collision(Data data, double here_x, double here_y, double here_vx, double here_vy, int here_k) {
	for (int i = 0; i < data.obs_n; i++) {
...
void Controller::feasible_domain(Vehicle v, Infrastructure r, int it) {
	...
	this->vymin.clear(); this->vymax.clear(); this->NVY.clear(); this->VY.clear();
	...
	/* vx domain */
	this->dvx = this->dux; this->cvx = this->cvx0;
	for (int k = 0; k < this->numsteps + 1; k++) {
		if (fmod(v.init_vx[numsteps-1], this->dvx) != 0) {
	...
	/* vy domain */
	this->dvy = this->duy; this->cvy = this->cvy0;
	for (int k = 0; k < this->numsteps + 1; k++) {
		this->vymin.push_back(MAX(v.init_vy[k] - this->cvy * this->dvy, v.getVYmin()));
		this->vymax.push_back(MIN(v.init_vy[k] + this->cvy * this->dvy, v.getVYmax()));

		this->NVY.push_back((int)ceil((this->vymax[k] - this->vymin[k]) / this->dvy) + 1);
		
		// vector<double> temp;
		for (int ivy = 0; ivy < this->NVY[k]; ivy++) {
			temp.push_back(this->vymin[k] + this->dvy * ivy);
		}
		this->VY.push_back(temp);
		temp.clear();
	}
	maxNVY = *max_element(NVY.begin(), NVY.end());

	/* x domain */
	this->dx = 0.5 * this->dux;
	...
	/* y domain */
	this->dy = 0.5 * this->duy;
	this->cy = this->cvy0 * this->cym;
	for (int k = 0; k < this->numsteps + 1; k++) {	
...
int Controller::discretizexv(node_t *state, int k) {
	...
	ivy = (int)round((state->vy - this->VY[k][0]) / this->dvy);
	...
	for (int k = 0; k < this->numsteps + 1; k++) {
		...
		for (int ix = 0; ix < this->maxNX; ix++) {
			...
			for (int iy = 0; iy < this->maxNY; iy++) {
				...
				for (int ivx = 0; ivx < this->maxNVX; ivx++) {
					this->J[k][ix][iy][ivx] = (double *)calloc(sizeof(double), maxNVY);
					this->Ux[k][ix][iy][ivx] = (double *)calloc(sizeof(double), maxNVY);
					this->Uy[k][ix][iy][ivx] = (double *)calloc(sizeof(double), maxNVY);
				}
			}
		}
	}
...
void Controller::printSolution(Vehicle v, Data data, Infrastructure inf){

	node_t *nextptr;
	node_t *state;
	double ux, uy, real_cost = 0.0;

	data.ax.clear(); data.ay.clear();
	data.x.clear(); data.y.clear(); data.vx.clear(); data.vy.clear();
	opt_x.clear(); opt_y.clear(); opt_vx.clear(); opt_vy.clear();
	...
	initial.vx = v.getVX0();
	initial.vy = v.getVY0();
	initial.ix = initial.iy = initial.ivx = initial.ivy = 0;

	state = (node_t*)calloc(sizeof(node_t), this->numsteps + 1);
	state[0] = initial;

	if (data.DISPLAY) {
		fprintf(stderr, "k \t x \t\t y \t\t vx \t\t vy \t\t ux \t\t uy \t\t J \t\t real cost \n");
		fprintf(stderr, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	}
	for (int k = 0; k <= this->numsteps; k++){
		ux = ACCESS(Ux, state[k]); data.ax.push_back(ux);
		uy = ACCESS(Uy, state[k]); data.ay.push_back(uy);
		data.x.push_back(state[k].x); data.y.push_back(state[k].y); data.vx.push_back(state[k].vx); data.vy.push_back(state[k].vy);
		opt_x.push_back(state[k].x); opt_y.push_back(state[k].y); opt_vx.push_back(state[k].vx); opt_vy.push_back(state[k].vy);

		// real cost
		real_cost += 0.5*pow(ux, 2) + 0.5*pow(uy, 2) + 
			0.5*pow(state[k].vx - v.getDersiredSpeedX(), 2) + 0.5*pow(state[k].vy - v.getDersiredSpeedY(), 2);
...
void Controller::dddp(Vehicle v, Infrastructure r, Data data) {
	...
	while(true) {
			...
			for (int ix = 0; ix < ((k == 0) ? 1 : this->NX[k]); ix++) {
				...
				for (int iy = 0; iy < ((k == 0) ? 1 : this->NY[k]); iy++) {
					...
					for (int ivx = 0; ivx < ((k == 0) ? 1 : this->NVX[k]); ivx++) {
						...
						for (int ivy = 0; ivy < ((k == 0) ? 1 : this->NVY[k]); ivy++) {
							vy = ((k==0) ? vy0 : this->VY[k][ivy]);

							// initialize current node
							node_t here = { 0 };
							here.k = k;
							here.x = x; here.y = y;
							here.vx = vx; here.vy = vy; 
							here.ix = ((k == 0) ? 0 : ix); here.iy = ((k == 0) ? 0 : iy); 
							here.ivx = ((k == 0) ? 0 : ivx); here.ivy = ((k == 0) ? 0 : ivy);
							
							if (here.ix > this->NX[k] || here.iy > this->NY[k] || here.ivx > this->NVX[k] || here.ivy > this->NVY[k]) {
							...
							if (k == this->numsteps) {
								if (v.collision(data, here.x, here.y, here.vx, here.vy, here.k) == 1)
									J[here.k][here.ix][here.iy][here.ivx][here.ivy] = DBL_MAX;
								else
									J[here.k][here.ix][here.iy][here.ivx][here.ivy] = 0.0;			

								Ux[here.k][here.ix][here.iy][here.ivx][here.ivy] = 0.0;
								Uy[here.k][here.ix][here.iy][here.ivx][here.ivy] = 0.0;
							...
							Jhere = &J[here.k][here.ix][here.iy][here.ivx][here.ivy];
							Uxhere = &Ux[here.k][here.ix][here.iy][here.ivx][here.ivy];
							Uyhere = &Uy[here.k][here.ix][here.iy][here.ivx][here.ivy];
							...
							*Jhere = DBL_MAX;
							...
							for (auto ux : UX) {
								for (auto uy : UY) {
									...
									next.y = here.y + here.vy*this->step + 0.5*uy*pow(this->step, 2);
									next.vx = here.vx + ux*this->step;
									next.vy = here.vy + uy*this->step;
									...
									// compute cost
									Jnext = &J[next.k][next.ix][next.iy][next.ivx][next.ivy];

									phi = 0.5*pow(ux, 2) + 0.5*pow(uy, 2) + pow(here.vx - v.getDersiredSpeedX(), 2) + pow(here.vy - v.getDersiredSpeedY(), 2);
									cost =  phi + *Jnext;
		...
		this->printSolution(v, data, r);

		it_toc = clock();
		it_cpu_time = (double)(it_toc - it_tic) / CLOCKS_PER_SEC;
		fprintf(stderr, "Iteration %d | Δux=Δvx: %.5f | Δx: %.5f | NX: %d | NY: %d | NVX: %d | NVY: %d | cost: %.5f | CPU time: %.4f \n", 
			it + 1, dux, dx, maxNX, maxNY, maxNVX, maxNVY, opt_J_it, it_cpu_time);
...
void Data::exportSimData(class Vehicle veh, class Infrastructure inf) {
...
void Controller::plotDomain(Vehicle v, int it) {
	...
	fputs("'vymin': [", fout);
	for (int k = 0; k < numsteps; k++)
		fprintf(fout, "%.4f,", vymin[k]);
	fputs("],\n", fout);

	fputs("'vymax': [", fout);
	for (int k = 0; k < numsteps; k++)
		fprintf(fout, "%.4f,", vymax[k]);
	fputs("],\n", fout);

	fputs("'vy': [\n", fout);
	for (int k = 0; k < numsteps; k++) {
		fputs("\t[", fout);
		for (int ivx = 0; ivx < NVY[k]; ivx++)
			fprintf(fout, "%.4f,", VY[k][ivx]);
		fputs("],\n", fout);
	}
	fputs("],\n", fout);
...
int main() {
	...
	Vehicle veh(data.x0, data.y0, data.vx0, data.vy0);
