    ...
    methods: {
        ...
        svg_initialize() {
            ...
            this.d3.svg_main = d3.select("#myDiv")
                ...
                .attr("height", roadh);
            // this.svg_load_data();
            this.threeEnvironment(this.sim);

        },

        threeEnvironment(sim) {
            var scene_el = document.getElementById("myDiv");
            var road = scene_el.getBoundingClientRect();
            var roadw = road.width;
            var roadh = road.height
            ...
            function Car(id) {
                ...
                var cabinColor = 0xcccccc;
                if (sim.isObs != undefined && sim.isObs[id])
                    cabinColor = color;

                const cabin = new THREE.Mesh(
                    // new THREE.BoxBufferGeometry( sim.Cx[id]*zoom*0.7, sim.Cy[id]*zoom*0.8, 1.2*zoom ), 
                    new THREE.BoxBufferGeometry( sim.Cx[id]*zoom*0.5, sim.Cy[id]*zoom*0.8, 1.2*zoom ), 
                    [
            ...
            function SelectedMarker() {
                ...
                const createSection = color => new THREE.Mesh(
                    ...
                    new THREE.BoxBufferGeometry( 2.0*sim.attraction_x*zoom, 2.0*sim.attraction_y*zoom, 0.1*zoom ), 
                    new THREE.MeshPhongMaterial( { color } )
                );
            ...
            function Boundary() {
                var y = [];
                var length = sim.roadlength;
                var slp = 0.2, swp1 = 200.0, swp2 = 300.0, sWidth = 5.0;
                for (var i = 1; i < length; i++) {
                    x = i;
                    yMin = -((-sWidth/(1.0 + Math.exp(-slp*(x - swp1)))) + (sWidth/(1.0 + Math.exp(-slp*(x - swp2)))))
                    y.push(yMin);
                }
                const blocks = new THREE.Group();
                                
                const createSection = (color, width) => new THREE.Mesh(
                    new THREE.BoxBufferGeometry( 1*zoom, (1)*zoom, 1.0*zoom ), 
                    new THREE.MeshPhongMaterial( { color } )
                );
                ...
                for (var i = 1; i < length; i++) {
                    ...
                    block[i].position.x = i*zoom;
                    block[i].position.y = (y[i]-(sim.roadwidth/2)-(1/2))*zoom;
                    blocks.add(block[i]);
                }
            ...
            function Lane(index) {
                ...
                switch(this.type) {
                    ...
                    case 'forest': {
                        ...
                        var treeSpacing = Math.min(200.0, sim.roadlength);
            ...
            function animate() {    
                ...
                var tweenTime = 0.25; // conf.data.sim.Step; // (conf.data.sim.Step*1000/conf.data.play.speed);
                if ( count_time < sim.k) {
                        ...
                        if(lane.type === 'car') {
                            lane.vechicles.forEach((vechicle, id )=> {
                                ...
                                if (conf.data.vehSelect.id != null && conf.data.vehSelect.id == sim.id[id]) {
                                    ...
                                    if (conf.data.vehSelect.marker == false && conf.data.vehSelect.id == sim.leaderID + 1) {
                                        conf.data.vehSelect.marker = true
                                        var myMarker = new SelectedMarker();
                                        
                                        myMarker.position.x = vechicle.position.x - (sim.attraction_x - sim.Cx[id])*zoom;
                                        myMarker.position.y = vechicle.position.y;
                                        myMarker.position.z = vechicle.position.z;
                                       
                                        myMarker.name = "myMarker";
                                        scene.add( myMarker );
                                    }
                                    else if (conf.data.vehSelect.marker == true){
                                        var mark = scene.getObjectByName("myMarker");
                                        mark.position.x = vechicle.position.x - (sim.attraction_x - sim.Cx[id])*zoom;
                                        mark.position.y = vechicle.position.y;
                                        mark.position.z = vechicle.position.z;
                                    }
        ...
        replaceSimFile(e) {            
            this.replacejscssfile("data/sim.js","data/sim_0.js","js");
        },
        ...
        createjscssfile(filename, filetype){
            if (filetype=="js"){ //if filename is a external JavaScript file
                ...
                // MYPART: assign the new values to this.sim
                fileref.onload = function () {
                    console.log("loaded");
                    window.conf.data.sim = window.sim;
                    
                    window.conf.methods.svg_initialize();
                };
